...
CREATE OR REPLACE FUNCTION locationToVID(location double precision[],
  ...
  BEGIN
    RETURN nearest_node(location[1], location[2], costname, rcostname, where_clause);
  END ;
$$ LANGUAGE 'plpgsql' ;
...
CREATE OR REPLACE FUNCTION isochroneGenerator(
  ...
  BEGIN
    ...
    isochrone_query := concat('SELECT dd.seq AS id, ST_X(v.the_geom) AS x, ST_Y(v.the_geom) AS y FROM pgr_drivingDistance($niv2$', graph_query, '$niv2$, ', locationToVID(location, costname, rcostname, where_clause), ', ', costValue, ') AS dd INNER JOIN ways_vertices_pgr AS v ON dd.node = v.id');
