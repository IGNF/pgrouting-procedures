    ...
    CASE algo
      ...
      WHEN 'astar' THEN
        IF array_length(coordinatesTable, 1) > 2 THEN
          ...
          LOOP
            RETURN QUERY SELECT * FROM coord_astar(m,costname,rcostname,attributes_query) ;
          END LOOP;
        ELSE
          RETURN QUERY SELECT * FROM coord_astar(coordinatesTable,costname,rcostname,attributes_query) ;
        END IF;
      WHEN 'trsp' THEN
        RETURN QUERY SELECT * FROM coord_trspVertices(coordinatesTable,costname,rcostname,attributes_query) ;
      ELSE
        RETURN QUERY SELECT * FROM coord_dijkstra(coordinatesTable,costname,rcostname,attributes_query) ;
    END CASE;
    -- --
