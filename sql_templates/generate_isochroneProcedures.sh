...
CREATE OR REPLACE FUNCTION $SCHEMA.isochroneGenerator(
  ...
  BEGIN
    ...
    IF (direction = 'arrival') THEN
      graph_query := concat('SELECT id, source, target, ', rcostName,' AS cost, ', costName,' AS reverse_cost FROM $SCHEMA.ways', where_clause);
    ELSE
      graph_query := concat('SELECT id, source, target, ', costName,' AS cost, ', rcostName,' AS reverse_cost FROM $SCHEMA.ways', where_clause);
    END IF;
