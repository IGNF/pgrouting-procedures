...
CREATE OR REPLACE FUNCTION $SCHEMA.coord_trspEdges(coordinatesTable double precision[][], -- table des points dans l'ordre de parcours
  ...
  BEGIN
    ...
    final_query := concat('SELECT path.seq as seq, -1 * path.id1 as path_seq, path.id2 as node,
                            ...
                            '   $SCHEMA.ways.cost_m_', profile_name,'*cost/$SCHEMA.ways.',real_cost_name,
                            ' ELSE
                                $SCHEMA.ways.reverse_cost_m_', profile_name,'*cost/$SCHEMA.ways.',real_rcost_name,'
                            END as distance,',
                            'CASE
                              WHEN $SCHEMA.ways.', real_cost_name, ' > 0 THEN',
                            '   $SCHEMA.ways.cost_s_', profile_name,'*cost/$SCHEMA.ways.',real_cost_name,
                            ' ELSE
                                $SCHEMA.ways.reverse_cost_s_', profile_name,'*cost/$SCHEMA.ways.',real_rcost_name,'
                             END as duration,',
                            waysAttributesQuery,'
...
CREATE OR REPLACE FUNCTION $SCHEMA.shortest_path_pgrouting(coordinatesTable double precision[][], -- table des points dans l'ordre de parcours
  ...
  BEGIN
    ...
    where_clause := concat(' WHERE WHERE importance=''1'' OR (the_geom && (SELECT ST_Expand( ST_Extent(the_geom), 0.1 ) FROM $SCHEMA.ways WHERE id = ANY(\$niv3\$', eidtable, '\$niv3\$::int[])))');
    -- where_clause := '';
    IF constraints != ''
    THEN
